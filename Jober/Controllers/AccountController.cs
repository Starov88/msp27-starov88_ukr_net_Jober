...
using Jober.Models;
using Jober.Models.AccountViewModels;
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (!result.Succeeded && signedUser != null)
                {
                    result = await _signInManager.PasswordSignInAsync(signedUser, model.Password, model.RememberMe, lockoutOnFailure: false);
                }

                if (result.IsNotAllowed)
                {
                    ModelState.AddModelError(string.Empty, "Необходимо подтвердить Email");
                    return View(model);
                }
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                await _emailSender.SendEmailAsync(model.Email, "Сброс пароля",
                   $"Пожалуйста, подтвердите смену пароля нажав на <a href='{callbackUrl}'>эту ссылку.</a>");
                return RedirectToAction(nameof(ForgotPasswordConfirmation));
            }
